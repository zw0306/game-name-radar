...
const els={
  ...
  sourceCount:document.querySelector('#sourceCount'),newCount:document.querySelector('#newCount'),
  onlineCount:document.querySelector('#onlineCount'),wikiCount:document.querySelector('#wikiCount'),
  sourceChips:document.querySelector('#sourceChips'),body:document.querySelector('#resultBody'),
  empty:document.querySelector('#emptyState'),emptyDetail:document.querySelector('#emptyDetail'),
  search:document.querySelector('#searchInput'),siteType:document.querySelector('#siteTypeFilter'),
  recommendation:document.querySelector('#recommendationFilter'),time:document.querySelector('#timeFilter'),
  refresh:document.querySelector('#refreshBtn'),export:document.querySelector('#exportBtn'),toast:document.querySelector('#toast'),
};

let candidates=[];let report={};let statuses=loadStatuses();
const LABELS={independent:'明显上涨·适合独立站',page:'稳定需求·适合专题页',watch:'继续观察',reject:'不建议做',pending:'等待验证',error:'验证失败'};
...
function serp(c){const query=siteType(c)==='wiki'?`"${c.gameName}" wiki guide walkthrough`:`"${c.gameName}" game play online`;return`https://www.google.com/search?q=${encodeURIComponent(query)}`}
...
function filtered(){
  ...
  const typeFilter=els.siteType.value;
  const filter=els.recommendation.value;
  const days=els.time.value==='all'?Infinity:Number(els.time.value);
...
function renderSources(){
  ...
  if(!logs.length){els.sourceChips.innerHTML='<span class="muted">等待后台自动扫描。</span>';return}
  for(const log of logs){
    const chip=document.createElement('span');chip.className=`source-chip ${log.ok?'':'error'}`;
    const name=document.createElement('b');name.textContent=log.sourceName||log.sourceId;
    const detail=document.createElement('small');detail.textContent=log.ok?`${log.total||0}项 · 新增${log.added||0}`:log.error||'失败';
    chip.append(name,detail);els.sourceChips.append(chip);
  }
}
...
function renderSeo(c){
  const seo=c.seo||{},box=document.createElement('div');box.className='judgement';box.append(judgementLabel(`SEO意图分 ${seoScore(c)}`));
  for(const text of (seo.reasons||[]).filter(x=>!x.startsWith('适合')&&!x.startsWith('不建议')).slice(0,2))box.append(small(text));
  if(Number.isFinite(seo.exactGameRatio))box.append(small(`主词游戏 ${Math.round(seo.exactGameRatio*100)}%${seo.exactNewsRatio>0?` · 新闻 ${Math.round(seo.exactNewsRatio*100)}%`:''}${seo.exactNonGameRatio>0?` · 非游戏 ${Math.round(seo.exactNonGameRatio*100)}%`:''} · 歧义 ${seo.nameRisk??'—'}`));
  return box;
}

function renderFast(c){
  const fast=c.fast||{},box=document.createElement('div');box.className='judgement';box.append(judgementLabel(`${FAST_LABELS[fast.classification]||'等待快速验证'} · ${fastScore(c)}分`));
  if(Number.isFinite(fast.sourceAdded24h))box.append(small(`24h新增 ${fast.sourceAdded24h}源 · 48h ${fast.sourceAdded48h||0}源${fast.bestRank?` · 最佳排名 ${fast.bestRank}`:''}`));
  if((fast.newSuggestionCount||0)||(fast.newSerpPageCount||0))box.append(small(`新增补全 ${fast.newSuggestionCount||0} · 新增SERP ${fast.newSerpPageCount||0}`));
  if(fast.youtubeEnabled)box.append(small(`YouTube 7天 ${fast.youtubeVideos||0}视频 · ${fast.youtubeChannels||0}频道 · ${Number(fast.youtubeViews||0).toLocaleString()}播放`));
  if((fast.reasons||[])[0])box.append(small(fast.reasons[0]));
  return box;
}

function renderTrend(c){
  const trend=c.trend||{},box=document.createElement('div');box.className='judgement trend-judgement';box.append(judgementLabel(`${TREND_LABELS[trend.classification]||'等待趋势验证'} · ${trendScore(c)}分`));
  if(trend.marketStatus==='global-rising')box.append(small(`全球${TREND_LABELS[trend.globalClassification]||'上涨'}，美国尚未同步`));
  else if(trend.marketStatus==='us-rising')box.append(small('美国趋势已明显上涨'));
  if(trend.keywordFreshness)box.append(small(`关键词历史：${FRESHNESS_LABELS[trend.keywordFreshness]||trend.keywordFreshness}${trend.entityConflict?' · 可能来自其他实体':''}`));
  if(Number.isFinite(trend.ratio7))box.append(small(`美国主词7天≈${trend.anchor||'itch io'}的 ${(trend.ratio7*100).toFixed(1)}% · 动量 ${trend.sevenDay?.momentum??'—'}x`));
  if(Number.isFinite(trend.ratio30))box.append(small(`美国主词30天≈${trend.anchor||'itch io'}的 ${(trend.ratio30*100).toFixed(1)}% · 动量 ${trend.thirtyDay?.momentum??'—'}x`));
  if(Number.isFinite(trend.globalRatio30))box.append(small(`全球30天≈基准的 ${(trend.globalRatio30*100).toFixed(1)}%`));
  const signal=(trend.reasons||[]).find(text=>/上涨|Breakout|尖峰|不是新出现|其他实体/.test(text));
  if(signal)box.append(small(signal));else if(!Number.isFinite(trend.ratio7))box.append(small(trend.reasons?.[0]||'快速层通过后才进入Google Trends'));
  return box;
}

function renderAdvice(c){
  const cls=classification(c),type=siteType(c),box=document.createElement('div');box.className='judgement';
  const typeBadge=document.createElement('span');typeBadge.className=`recommendation ${cls}`;typeBadge.textContent=TYPE_LABELS[type]||TYPE_LABELS.pending;box.append(typeBadge);
  box.append(small(LABELS[cls]||cls));
  if(c.siteType?.reasons?.[0])box.append(small(c.siteType.reasons[0]));
  if(type==='online')box.append(small('适合HTML5／iframe方向，仍需确认嵌入权限'));
  if(type==='wiki')box.append(small('适合攻略、Wiki、地图、角色与联机教程'));
  return box;
}
...
function render(){
  ...
  for(const c of list.slice(0,500)){
    ...
    const gameTd=document.createElement('td'),title=document.createElement('strong'),url=document.createElement('a'),meta=document.createElement('span');title.className='game-title';title.textContent=c.gameName;url.className='game-url';url.href=c.sources?.[0]?.url||'#';url.target='_blank';url.rel='noopener noreferrer';url.textContent=c.sources?.[0]?.url||'';meta.className='game-meta';meta.textContent=`发现分 ${c.discoveryScore??'—'} · ${c.sources?.length||0}个来源 · ${fmtDate(c.firstSeen)}`;gameTd.append(title,url,meta);
    const adviceTd=document.createElement('td');adviceTd.append(renderAdvice(c));
...
els.export.addEventListener('click',()=>{
  const rows=[['Game Name','Site Type','Browser Playable','Iframe Likely','Final Score','Recommendation','SEO Score','Fast Score','YouTube Videos','YouTube Channels','YouTube Views','Trend Class','US 7d vs Anchor','US 30d vs Anchor','Global Trend Class','Discovery Score','Sources','URL'],...filtered().map(c=>[c.gameName,TYPE_LABELS[siteType(c)],c.siteType?.browserPlayable??'',c.siteType?.iframeLikely??'',finalScore(c),LABELS[classification(c)]||classification(c),seoScore(c),fastScore(c),c.fast?.youtubeVideos??'',c.fast?.youtubeChannels??'',c.fast?.youtubeViews??'',c.trend?.classification??'',c.trend?.ratio7??'',c.trend?.ratio30??'',c.trend?.globalClassification??'',c.discoveryScore||0,(c.sources||[]).map(s=>s.name).join(' | '),c.sources?.[0]?.url||''])];
