...
function baseCandidate(overrides = {}) {
  return {
    ...
    fast: { modelVersion: 2, classification: 'pass', score: 60 },
    ...overrides,
  };
}
...
test('classifies strategic watch candidate as secondary tier', () => {
  const candidate = baseCandidate({ fast: { modelVersion: 2, classification: 'watch', score: 30 } });
...
test('skips candidates with fresh trend data', () => {
  const candidate = baseCandidate({
    trend: { modelVersion: 4, checkedAt: new Date().toISOString(), classification: 'rising', score: 80 },
  });
  assert.equal(classifyTrendTier(candidate), null);
});
...
test('builds queue in strong, secondary, strategic order', () => {
  ...
  const secondary = baseCandidate({ id: 'secondary', fast: { modelVersion: 2, classification: 'watch', score: 30 } });
  const strategic = baseCandidate({
    id: 'strategic',
    seo: { modelVersion: 5, classification: 'page', score: 40, nameRisk: 15, entityConflict: false },
    fast: { modelVersion: 2, classification: 'weak', score: 22 },
  });
