...
test('passes cross-platform social promotion', () => {
  const result = analyzeOnlineSocialBuzz({ social: { providers: {
    ...
    reddit: provider({ postCount: 5, subredditCount: 3, totalScore: 200, totalComments: 100, recent24h: 1 }),
    x: {},
    tiktok: {},
  } } });
...
test('allows one very strong provider to pass', () => {
  const result = analyzeOnlineSocialBuzz({ social: { providers: {
  ...
  assert.ok(['pass', 'viral'].includes(result.classification));
  assert.equal(result.allowsIndependent, true);
});
