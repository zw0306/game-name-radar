...
const outputPath = path.join(root, 'data', 'apify-data-ops-trends-smoke.json');
const token = process.env.APIFY_API_TOKEN || '';
const actorId = process.env.APIFY_DATA_OPS_TRENDS_ACTOR_ID || 'data_ops_main~google-trends';
...
if (existing?.ok && Number(existing?.itemCount || 0) > 0 && existing?.actorId === actorId) {
  console.log('Resilient Apify Trends smoke test already passed; skipping.');
  process.exit(0);
}
...
try {
  ...
  result = {
    ...
    sample: sanitize(items.slice(0, 5)),
  };
  if (!result.ok) result.error = 'Actor completed but returned no dataset items';
  console.log(`Resilient Apify Trends smoke test: ${items.length} item(s), usable=${result.ok}.`);
} catch (error) {
  result = {
    checkedAt: new Date().toISOString(),
    configured: true,
    ok: false,
    actorId,
    error: error?.name === 'AbortError' ? 'Resilient Apify Trends smoke test timed out' : String(error?.message || error),
  };
