...
async function readUsage() {
  ...
  const usage = {
    ...
    dayUsed: Number(stored.dayUsed || 0),
...
async function ensureQuota() {
  ...
  if (usage.totalUsed >= TOTAL_LIMIT || usage.dayUsed >= DAILY_LIMIT) {
    const error = new Error('Serper安全额度已用完');
    error.code = 'SERPER_QUOTA_GUARD';
    throw error;
  }
}
...
async function searchSerper(query) {
  ...
  try {
    const response = await fetch('https://google.serper.dev/search', {
      ...
      body: JSON.stringify({ q: query, gl: COUNTRY, hl: LANGUAGE, num: 10, autocorrect: true }),
    ...
    if (!response.ok) {
      ...
      if ([402, 429].includes(response.status) || /credit|quota|limit/i.test(String(message))) error.code = 'SERPER_QUOTA_GUARD';
      else error.code = 'SERPER_UNAVAILABLE';
      throw error;
    }
    await recordSuccess();
    ...
    if (error?.name === 'AbortError') {
      const timeoutError = new Error('Serper请求超时');
      timeoutError.code = 'SERPER_UNAVAILABLE';
      throw timeoutError;
    }
    throw error;
  } finally {
...
function sourceCount(candidate) {
  return new Set((candidate.sources || []).map((source) => source.sourceId || source.kind || source.url)).size;
}
...
function needsSeo(candidate) {
  ...
  return ['pending', 'error', 'watch'].includes(candidate.seo?.classification)
    || ['duckduckgo+autocomplete', 'brave+autocomplete'].includes(candidate.seo?.provider)
    || candidate.seo?.provider?.startsWith('google-cse-');
}
...
function isUseful(candidate) {
  ...
  return strategic || count >= 2 || (recent && Number(candidate.discoveryScore || 0) >= 4 && risk <= 17);
}

function priority(candidate) {
  const kinds = sourceKinds(candidate);
  const count = sourceCount(candidate);
  let score = Number(candidate.discoveryScore || 0) * 5
    + Math.max(0, 25 - estimateNameRisk(candidate.gameName || ''))
    + count * 10;
  ...
  if (kinds.has('steam-popular-new')) score += 30;
...
function refreshRecommendation(candidate) {
  const seo = candidate.seo?.classification || 'pending';
  const fast = candidate.fast?.classification || 'pending';
  if (seo === 'error') candidate.recommendation = 'error';
  else if (seo === 'reject' || candidate.seo?.entityConflict || ['reject', 'weak'].includes(fast)) candidate.recommendation = 'reject';
...
for (const candidate of API_KEY ? queue : []) {
  try {
    ...
    candidate.seo = {
      ...
      queryName: name,
      searchRequests: 1,
      serperUsage: await usageSummary(),
      ...calculateSeoVerdict({
        gameName: name,
    ...
    if (error.code === 'SERPER_QUOTA_GUARD') {
      quotaStopped = true;
      break;
    }
    errors += 1;
...
await fs.writeFile(reportPath, JSON.stringify({
  ...report,
  seoProvider: API_KEY ? 'serper-google-search' : report.seoProvider,
  serperConfigured: Boolean(API_KEY),
  serperUsage: await usageSummary(),
  serperVerification: {
    limit: VERIFY_LIMIT,
...
console.log(`Serper SEO complete: ${verified} verified, ${errors} errors, quota stopped ${quotaStopped}.`);
