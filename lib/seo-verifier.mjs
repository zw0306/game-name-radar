...
const GAME_DOMAINS = [
  ...
  'gamejolt.com','newgrounds.com','kongregate.com','armorgames.com','miniclip.com','fandom.com','youtube.com','youtu.be',
  'reddit.com','tiktok.com','play.google.com','apps.apple.com','roblox.com','github.io','gx.games'
];
const NEWS_DOMAINS = [
...
const GAME_TERMS = /\b(game|games|play|online|gameplay|walkthrough|browser|html5|itch|steam|download|apk|guide|wiki|controls|ending)\b/i;
...
const HIGH_RISK_TERMS = /\b(military|war|airstrike|attack|election|president|government|weather|hurricane|earthquake|disease|virus|stock|crypto|bitcoin|football|basketball|parking|supermarket)\b/i;
const GENERIC_WORDS = new Set([
  ...
  'marble','block','blood','alien','wool','empire','game','online','3d','2d','idle','clicker','puzzle','adventure','hero','heroes'
]);
...
function classifyResults(results) {
  let game=0,news=0,other=0;
  for(const result of results.slice(0,10)){
    const host=hostOf(result.url),text=`${result.title} ${result.snippet}`;
    const isGame=domainMatches(host,GAME_DOMAINS)||GAME_TERMS.test(text);
    const isNews=domainMatches(host,NEWS_DOMAINS)||(!isGame&&NEWS_TERMS.test(text));
    if(isGame) game+=1; else if(isNews) news+=1; else other+=1;
  }
  const total=Math.max(1,game+news+other);
  return {game,news,other,total,gameRatio:game/total,newsRatio:news/total};
}

function nameRisk(name='') {
  ...
  if(HIGH_RISK_TERMS.test(name)) risk+=8;
...
export function calculateSeoVerdict({gameName,exactResults=[],gameResults=[],suggestions=[],discoveryScore=0}) {
  const exact=classifyResults(exactResults),withGame=classifyResults(gameResults);
  const lower=gameName.toLowerCase();
  const suggestionGame=suggestions.filter(item=>item.toLowerCase().includes(lower)&&GAME_TERMS.test(item)).length;
  const suggestionExact=suggestions.some(item=>item.toLowerCase()===lower);
  const risk=nameRisk(gameName);

  let score=0;
  score+=Math.round(exact.gameRatio*38);
  score+=Math.round(withGame.gameRatio*18);
  score+=suggestionGame?12:0;
  score+=suggestionExact?5:0;
  score+=Math.min(10,Math.round(discoveryScore/2));
  score-=Math.round(exact.newsRatio*38);
  score-=risk;
  score=Math.max(0,Math.min(100,score));

  let classification='watch';
  if(exact.newsRatio>=0.5||(exact.game===0&&risk>=18)) classification='reject';
  else if(score>=68&&exact.gameRatio>=0.35) classification='independent';
  else if(score>=42&&(withGame.gameRatio>=0.45||suggestionGame>0)) classification='page';
  else if(score<22) classification='reject';
  ...
  if(exact.newsRatio>=0.4) reasons.push('新闻或非游戏意图过强');
  ...
  return {
    ...
    gameQueryGameRatio:Number(withGame.gameRatio.toFixed(2)),suggestionGameCount:suggestionGame,
    suggestions:suggestions.slice(0,10),nameRisk:risk,
    exactResultCount:exact.total,gameResultCount:withGame.total
  };
}
...
export async function verifyGameKeyword(gameName, discoveryScore=0) {
  const [exactResults,gameResults,suggestions]=await Promise.all([
    searchDuck(`"${gameName}"`),
    searchDuck(`"${gameName}" game play online`),
    fetchSuggestions(gameName)
  ]);
  return {
    checkedAt:new Date().toISOString(),provider:'duckduckgo+autocomplete',
    ...calculateSeoVerdict({gameName,exactResults,gameResults,suggestions,discoveryScore})
  };
}
