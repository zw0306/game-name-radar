...
export function calculateSeoVerdict({gameName,exactResults=[],gameResults=[],suggestions=[],discoveryScore=0}) {
  ...
  else if(score>=55&&risk<=12&&exact.gameRatio>=0.5&&exact.game>=2&&(withGame.gameRatio>=0.5||suggestionGame>0)) classification='independent';
  else if(score>=30&&risk<=24&&(withGame.gameRatio>=0.45||suggestionGame>0||exact.gameRatio>=0.5)) classification='page';
  else if(score<18||risk>=25) classification='reject';
