...
export function cleanGameName(value='') {
  return value
    .replace(/\s*\[(?:free|paid|demo|prototype|alpha|beta|jam|action|adventure|horror|puzzle|rpg|windows|mac|macos|linux|android|html5|web|browser|downloadable)(?:\s+game)?\]\s*/gi,' ')
    .replace(/\s*\((?:demo|prototype|alpha|beta|playtest|game jam version)\)\s*$/gi,'')
...
function classifyResults(results) {
  ...
  const rawTotal=game+news+other;
  const total=Math.max(1,rawTotal);
  return {game,news,other,total,rawTotal,gameRatio:game/total,newsRatio:news/total};
}
...
export function calculateSeoVerdict({gameName,exactResults=[],gameResults=[],suggestions=[],discoveryScore=0}) {
  ...
  else if(score>=55&&exact.gameRatio>=0.5&&exact.game>=2&&(withGame.gameRatio>=0.5||suggestionGame>0)) classification='independent';
  else if(score>=30&&(withGame.gameRatio>=0.45||suggestionGame>0||exact.gameRatio>=0.5)) classification='page';
  else if(score<18) classification='reject';
