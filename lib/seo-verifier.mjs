...
async function searchBrave(query) {
  ...
  try {
    ...
    if(!response.ok)throw new Error(payload?.message||`Brave Search returned ${response.status}`);
...
export async function verifyGameKeyword(gameName, discoveryScore=0) {
  ...
  try {
    ...
    if(error.code==='BRAVE_QUOTA_GUARD'){
      return {checkedAt:new Date().toISOString(),status:'pending',provider:'brave+autocomplete',queryName:cleanedName,classification:'pending',score:0,entityConflict:false,nameRisk:estimateNameRisk(cleanedName),reasons:['Brave Search API今日安全额度已用完，等待下一轮继续验证'],braveSearchUsage:await getBraveSearchUsage()};
    }
    throw error;
  }
}
