...
function attachEvaluationReasons(candidate, market, wikiPrelaunch, channel) {
  const current = candidate.siteType || classifySiteType(candidate);
  const extraReasons = channel === 'wiki' ? wikiPrelaunch.reasons : [];
  const reasons = [
    ...extraReasons,
    market.reason,
    ...(current.reasons || []),
...
export function applyFinalRecommendation(candidate) {
  ...
  attachEvaluationReasons(candidate, market, wikiPrelaunch, channel);
  ...
  const wikiCompetitionOpen = market.competitionChecked
    && !['occupied', 'established', 'contested'].includes(market.status)
    && keywordFreshness !== 'existing';
  const wikiCanStandAlone = channel === 'wiki' && wikiPrelaunch.allowsIndependent && wikiCompetitionOpen;
  ...
  else if (usRising && channel === 'wiki' && wikiCanStandAlone && seoClass === 'independent' && fastClass === 'pass' && nameRisk <= 12) recommendation = 'independent';
  ...
  // 在线游戏和攻略游戏分别使用独立的硬门槛：
  // 在线游戏要求严格新词且可浏览器直接玩；攻略游戏要求Steam预发布热度、攻略需求和竞争空缺同时成立。
  if (recommendation === 'independent' && channel === 'online' && !market.allowsIndependent) recommendation = 'watch';
  ...
  const weights = channel === 'online'
    ? { seo: 0.28, fast: 0.34, trend: 0.34, global: 0.04, prelaunch: 0 }
    : { seo: 0.25, fast: 0.20, trend: 0.20, global: 0.05, prelaunch: 0.30 };
  ...
  if (channel === 'online' && !market.allowsIndependent && market.rising) finalScore = Math.min(finalScore, 59);
