import { classifySiteType, isOnlineSource, isWikiSource } from './site-type.mjs';
...
function sourceStats(candidate, nowMs) {
  ...
  const onlineHighQualityCount = onlineSources.filter((source) => ONLINE_HIGH_QUALITY_KINDS.has(source.kind) || ONLINE_HIGH_QUALITY_KINDS.has(source.sourceId)).length;
  const wikiHighQualityCount = wikiSources.filter((source) => WIKI_HIGH_QUALITY_KINDS.has(source.kind) || WIKI_HIGH_QUALITY_KINDS.has(source.sourceId)).length;
  const sharedHighQualityCount = list.filter((source) => SHARED_HIGH_QUALITY_KINDS.has(source.kind)).length;
  const onlinePlatformCount = new Set(onlineSources.map((source) => source.sourceId || source.kind || source.url)).size;
