...
function parseSitemapXml(text, baseUrl) {
  ...
  if (sitemapBlocks.length) {
    return {
      type: 'sitemap-index',
      children: sitemapBlocks
        .map((match) => normalizeUrl(extractTag(match[1], 'loc'), baseUrl))
        .filter(Boolean),
      entries: [],
    };
  }
...
export function parseRemoteDocument(text, baseUrl, requestedKind = 'auto') {
  ...
  if (/itch\.io/i.test(baseUrl) || /game_cell|class=["'][^"']*title/i.test(text)) {
    return parseItchHtml(text, baseUrl);
  }
...
function wordsFromSlug(slug) {
  const stopWords = new Set([
    'play', 'online', 'game', 'games', 'free', 'unblocked', 'html5', 'browser',
    'download', 'new', 'official', 'web', 'guide', 'walkthrough', 'wiki', 'codes',
  ]);
...
export function deriveGameName(entry) {
  ...
  try {
    ...
    const words = wordsFromSlug(decodeURIComponent(slug));
    return words
  ...
  if (parsed.type === 'sitemap-index' && depth < 2 && maxChildren > 0) {
    ...
    for (let index = 0; index < childUrls.length; index += 4) {
      ...
      for (const item of settled) {
        if (item.status === 'fulfilled') childResults.push(...item.value.entries);
      }
...
export function calculateCandidateScore(candidate) {
  ...
  for (const kind of kinds) {
    ...
    else if (kind === 'competitor-sitemap') score += 3;
