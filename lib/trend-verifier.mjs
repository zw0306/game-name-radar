...
async function reserveSerpApiRequest() {
  ...
  if (Number(usage.monthUsed || 0) >= SERPAPI_MONTHLY_LIMIT) {
    const error = new Error(`SerpApi monthly safety limit reached (${SERPAPI_MONTHLY_LIMIT})`);
    error.code = 'SERPAPI_QUOTA_GUARD';
    error.apiRequests = 0;
    throw error;
  }
  if (Number(usage.dayUsed || 0) >= SERPAPI_DAILY_LIMIT) {
    const error = new Error(`SerpApi daily safety limit reached (${SERPAPI_DAILY_LIMIT})`);
    error.code = 'SERPAPI_QUOTA_GUARD';
    error.apiRequests = 0;
    throw error;
  }
  const updated = {
    ...usage,
    monthUsed: Number(usage.monthUsed || 0) + 1,
    dayUsed: Number(usage.dayUsed || 0) + 1,
...
export async function getSerpApiUsage() {
  ...
  const usage = await readUsage();
  return { enabled: true, ...usage, monthlyLimit: SERPAPI_MONTHLY_LIMIT, dailyLimit: SERPAPI_DAILY_LIMIT };
}
...
async function fetchSerpApiComparison(keyword, anchor = DEFAULT_ANCHOR, market = 'US') {
  const usage = await reserveSerpApiRequest();
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), SERPAPI_TIMEOUT_MS);
  const params = new URLSearchParams({
    engine: 'google_trends',
    q: [cleanQuery(keyword), cleanQuery(`${keyword} game`), cleanQuery(anchor)].join(','),
  ...
  try {
    ...
    if (!response.ok || payload.error || payload?.search_metadata?.status === 'Error') {
      throw new Error(payload.error || `SerpApi returned HTTP ${response.status}`);
    }
    return {
      candidate: parseSerpApiTimeline(payload, 0),
      qualified: parseSerpApiTimeline(payload, 1),
      anchor: parseSerpApiTimeline(payload, 2),
      searchId: payload?.search_metadata?.id || null,
      cached: Boolean(payload?.search_metadata?.cached),
      usage,
    };
  } catch (error) {
...
async function fetchFreeComparison(keyword, days, anchor = DEFAULT_ANCHOR, market = 'US') {
  const endTime = new Date();
  const startTime = new Date(endTime.getTime() - days * 86400000);
  const isWorldwide = market === 'WORLDWIDE';
  const raw = await googleTrends.interestOverTime({
    keyword: [keyword, `${keyword} game`, anchor],
    startTime,
    endTime,
    geo: isWorldwide ? '' : market,
    hl: 'en-US',
    timezone: 240,
  });
  return {
    candidate: parseGoogleTimeline(raw, 0),
    qualified: parseGoogleTimeline(raw, 1),
    anchor: parseGoogleTimeline(raw, 2),
  };
}

function verdictFromNinety(gameName, ninety, anchor) {
  const thirtyCount = Math.min(30, ninety.candidate.length);
  const sevenCount = Math.min(7, ninety.candidate.length);
  return calculateTrendVerdict({
    gameName,
    sevenDayCandidate: ninety.candidate.slice(-sevenCount),
    sevenDayQualified: ninety.qualified.slice(-sevenCount),
    sevenDayAnchor: ninety.anchor.slice(-sevenCount),
    thirtyDayCandidate: ninety.candidate.slice(-thirtyCount),
    thirtyDayQualified: ninety.qualified.slice(-thirtyCount),
    thirtyDayAnchor: ninety.anchor.slice(-thirtyCount),
...
function verdictFromFreeComparisons(gameName, thirty, ninety, anchor) {
...
async function fetchMarketVerdict(gameName, anchor, market) {
  if (SERPAPI_API_KEY) {
    const ninety = await fetchSerpApiComparison(gameName, anchor, market);
    return { verdict: verdictFromNinety(gameName, ninety, anchor), apiRequests: 1, searchIds: ninety.searchId ? [ninety.searchId] : [] };
  }
  const thirty = await fetchFreeComparison(gameName, 30, anchor, market);
  const ninety = await fetchFreeComparison(gameName, 90, anchor, market);
  return { verdict: verdictFromFreeComparisons(gameName, thirty, ninety, anchor), apiRequests: 0, searchIds: [] };
}
...
export async function verifyTrendDemand(gameName, anchor = DEFAULT_ANCHOR) {
  ...
  const searchIds = [];
  try {
    const usResult = await fetchMarketVerdict(gameName, anchor, 'US');
    apiRequests += usResult.apiRequests;
    searchIds.push(...usResult.searchIds);
    const primary = usResult.verdict;

    let worldwide = null;
    if (TARGET_MARKET === 'US_GLOBAL' && !['rising', 'breakout'].includes(primary.classification)) {
      if (!SERPAPI_API_KEY) await sleep(1500);
      try {
        const globalResult = await fetchMarketVerdict(gameName, anchor, 'WORLDWIDE');
        apiRequests += globalResult.apiRequests;
        searchIds.push(...globalResult.searchIds);
        worldwide = globalResult.verdict;
      } catch (error) {
        if (error.code !== 'SERPAPI_QUOTA_GUARD') throw error;
      }
    }

    const globalRising = ['rising', 'breakout'].includes(worldwide?.classification);
    const marketStatus = ['rising', 'breakout'].includes(primary.classification)
      ? 'us-rising'
      : globalRising
        ? 'global-rising'
        : 'no-rising';
    ...
    return {
      ...
      provider: SERPAPI_API_KEY ? 'serpapi' : 'google-trends-api',
      apiRequests,
      searchIds,
      serpApiUsage: await getSerpApiUsage(),
      targetMarket: TARGET_MARKET,
      primaryMarket: 'US',
      referenceMarket: 'WORLDWIDE',
      marketStatus,
      globalClassification: worldwide?.classification || null,
      globalScore: worldwide?.score ?? null,
      globalRatio7: worldwide?.ratio7 ?? null,
      globalRatio30: worldwide?.ratio30 ?? null,
      globalRising7: worldwide?.rising7 ?? false,
      globalRising30: worldwide?.rising30 ?? false,
      globalBreakout7: worldwide?.breakout7 ?? false,
      globalBreakout30: worldwide?.breakout30 ?? false,
      globalKeywordFreshness: worldwide?.keywordFreshness || null,
      worldwide,
      ...primary,
      reasons,
    };
