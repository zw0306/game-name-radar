...
async function fetchComparison(keyword, days, anchor = DEFAULT_ANCHOR, market = 'US') {
...
function verdictFromComparisons(gameName, thirty, ninety, anchor) {
...
export async function verifyTrendDemand(gameName, anchor = DEFAULT_ANCHOR) {
  const usThirty = await fetchComparison(gameName, 30, anchor, 'US');
  const usNinety = await fetchComparison(gameName, 90, anchor, 'US');
  const primary = verdictFromComparisons(gameName, usThirty, usNinety, anchor);

  let worldwide = null;
  if (TARGET_MARKET === 'US_GLOBAL' && !['rising', 'breakout'].includes(primary.classification)) {
    await sleep(1500);
    const globalThirty = await fetchComparison(gameName, 30, anchor, 'WORLDWIDE');
    const globalNinety = await fetchComparison(gameName, 90, anchor, 'WORLDWIDE');
    worldwide = verdictFromComparisons(gameName, globalThirty, globalNinety, anchor);
  }
