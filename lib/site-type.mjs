...
export function classifySiteType(candidate = {}) {
  ...
  const onlinePlatforms = new Set(onlineSources.map((source) => source.sourceId || hostname(source.url) || source.kind).filter(Boolean));
  const premiumOnlineCount = onlineSources.filter((source) => PREMIUM_ONLINE_SOURCE_IDS.has(source.sourceId)).length;
